/* Rmagneto.h
	Various functions from http://www.mymathlib.com/matrices/
	For use with Rmagneto.c, a program to calculate bias corrections for
                                   ...
                                   double trace, double det, double *S, int n)
{
   ...
   c = pH[n + n + k + 1];
   s = fabs(a) + fabs(b) + fabs(c);	// fabs = absolute value of floating point number
   a /= s;
   b /= s;
   c /= s;
